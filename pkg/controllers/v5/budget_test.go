...
func createTestBudget(t *testing.T, c v5.BudgetEditable, expectedStatus ...int) v5.BudgetCreatedResponse {
	// Default to 201 Created as expected status
	if len(expectedStatus) == 0 {
		expectedStatus = append(expectedStatus, http.StatusCreated)
	}

	body := []v5.BudgetEditable{
		c,
	}

	r := test.Request(t, http.MethodPost, "http://example.com/v5/budgets", body)
	test.AssertHTTPStatus(t, &r, expectedStatus...)

	var a v5.BudgetCreateResponse
	...
	return v5.BudgetCreatedResponse{}
}
...
func (suite *TestSuiteStandard) TestBudgetsDBClosed() {
	tests := []struct {
	...
	}{
		{
			"Creation fails",
			func(t *testing.T) {
				createTestBudget(t, v5.BudgetEditable{}, http.StatusInternalServerError)
			},
		},
		{
			"GET fails",
			func(t *testing.T) {
				recorder := test.Request(t, http.MethodGet, "http://example.com/v5/budgets", "")
				test.AssertHTTPStatus(t, &recorder, http.StatusInternalServerError)

				var response httperror.Error
				test.DecodeResponse(t, &recorder, &response)
				assert.Contains(t, response.Message, models.ErrGeneral.Error())
			},
		},
	}
...
func (suite *TestSuiteStandard) TestBudgetOptions() {
	tests := []struct {
		name   string
		id     string // path at the /v5/budgets endpoint to test
		...
		{"Budget exists", createTestBudget(suite.T(), v5.BudgetEditable{}).Data.ID.String(), http.StatusNoContent},
	}

	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			path := fmt.Sprintf("%s/%s", "http://example.com/v5/budgets", tt.id)
...
func (suite *TestSuiteStandard) TestBudgetsGetSingle() {
	budget := createTestBudget(suite.T(), v5.BudgetEditable{})
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, tt.method, fmt.Sprintf("http://example.com/v5/budgets/%s", tt.id), "")

			var budget v5.BudgetResponse
...
func (suite *TestSuiteStandard) TestBudgetsGetFilter() {
	_ = createTestBudget(suite.T(), v5.BudgetEditable{
	...
	var re v5.BudgetListResponse
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, http.MethodGet, fmt.Sprintf("http://example.com/v5/budgets?%s", tt.query), "")
...
func (suite *TestSuiteStandard) TestBudgetsCreateFails() {
	tests := []struct {
		name     string
		body     string
		testFunc func(t *testing.T, b httperror.Error) // tests to perform against the updated budget resource
	}{
		{"Broken Body", `{ "note": 2 }`, func(t *testing.T, e httperror.Error) {
			assert.Equal(t, "json: cannot unmarshal object into Go value of type []v5.BudgetEditable", e.Message)
		}},
		{"No body", "", func(t *testing.T, e httperror.Error) {
			assert.Equal(t, "the request body must not be empty", e.Message)
		}},
	}

	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, http.MethodPost, "http://example.com/v5/budgets", tt.body)
...
func (suite *TestSuiteStandard) TestBudgetsUpdate() {
	...
	var updatedBudget v5.BudgetResponse
...
func (suite *TestSuiteStandard) TestBudgetsUpdateFails() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			// Update budget
			recorder = test.Request(t, http.MethodPatch, fmt.Sprintf("http://example.com/v5/budgets/%s", tt.id), tt.body)
...
func (suite *TestSuiteStandard) TestBudgetsDelete() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			if tt.id == "" {
				// Create test budget
				b := createTestBudget(t, v5.BudgetEditable{})
				tt.id = b.Data.ID.String()
			}

			// Update budget
			recorder = test.Request(t, http.MethodDelete, fmt.Sprintf("http://example.com/v5/budgets/%s", tt.id), "")
			test.AssertHTTPStatus(t, &recorder, tt.status)
		})
	}
}
...
func (suite *TestSuiteStandard) TestBudgetsGetSorted() {
	b1 := createTestBudget(suite.T(), v5.BudgetEditable{
		Name: "Alphabetically first",
	})

	b2 := createTestBudget(suite.T(), v5.BudgetEditable{
		Name: "Second in creation, third in list",
	})

	b3 := createTestBudget(suite.T(), v5.BudgetEditable{
		Name: "First is alphabetically second",
	})

	b4 := createTestBudget(suite.T(), v5.BudgetEditable{
		Name: "Zulu is the last one",
	})

	r := test.Request(suite.T(), http.MethodGet, "http://example.com/v5/budgets", "")
	test.AssertHTTPStatus(suite.T(), &r, http.StatusOK)

	var budgets v5.BudgetListResponse
...
func (suite *TestSuiteStandard) TestBudgetsPagination() {
	for i := 0; i < 10; i++ {
		createTestBudget(suite.T(), v5.BudgetEditable{Name: fmt.Sprint(i)})
	}
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(suite.T(), http.MethodGet, fmt.Sprintf("http://example.com/v5/budgets?offset=%d&limit=%d", tt.offset, tt.limit), "")
			test.AssertHTTPStatus(suite.T(), &r, http.StatusOK)
