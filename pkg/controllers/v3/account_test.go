...
func (suite *TestSuiteStandard) createTestAccount(t *testing.T, account models.Account, expectedStatus ...int) v3.AccountResponse {
	...
	r := test.Request(t, http.MethodPost, "http://example.com/v3/accounts", body)
	test.AssertHTTPStatus(t, &r, expectedStatus...)

	var a v3.AccountCreateResponse
...
func (suite *TestSuiteStandard) TestAccountsDBClosed() {
	...
	tests := []struct {
		...
		},
		{
			"GET fails",
			func(t *testing.T) {
				recorder := test.Request(t, http.MethodGet, "http://example.com/v3/accounts", "")
...
func (suite *TestSuiteStandard) TestAccountsGetSingle() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, tt.method, fmt.Sprintf("http://example.com/v3/accounts/%s", tt.id), "")

			var account v3.AccountResponse
...
func (suite *TestSuiteStandard) TestAccountsGetFilter() {
	...
	tests := []struct {
		...
		query     string
		len       int
		checkFunc func(t *testing.T, accounts []v3.Account)
	}{
		...
		{"Not Archived", "archived=false", 3, func(t *testing.T, accounts []v3.Account) {
			for _, a := range accounts {
				assert.False(t, a.Archived)
			}
		}},
		{"Archived", "archived=true", 2, func(t *testing.T, accounts []v3.Account) {
			for _, a := range accounts {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			var re v3.AccountListResponse
			r := test.Request(t, http.MethodGet, fmt.Sprintf("/v3/accounts?%s", tt.query), "")
...
func (suite *TestSuiteStandard) TestAccountsCreateFails() {
	...
	tests := []struct {
		...
		testFunc func(t *testing.T, a v3.AccountCreateResponse) // tests to perform against the updated account resource
	}{
		{"Broken Body", `[{ "note": 2 }]`, http.StatusBadRequest, func(t *testing.T, a v3.AccountCreateResponse) {
		...
		}},
		{
			...
			func(t *testing.T, a v3.AccountCreateResponse) {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, http.MethodPost, "http://example.com/v3/accounts", tt.body)
			test.AssertHTTPStatus(t, &r, tt.status)
...
func (suite *TestSuiteStandard) TestAccountsUpdate() {
	...
	tests := []struct {
		...
		testFunc func(t *testing.T, a v3.AccountResponse) // tests to perform against the updated account resource
	}{
		{
			"Name, On Budget, Note",
			map[string]any{
			...
			func(t *testing.T, a v3.AccountResponse) {
...
func (suite *TestSuiteStandard) TestAccountsUpdateFails() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			// Update Account
			recorder = test.Request(t, http.MethodPatch, fmt.Sprintf("http://example.com/v3/accounts/%s", tt.id), tt.body)
...
func (suite *TestSuiteStandard) TestAccountsDelete() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			// Delete Account
			recorder = test.Request(t, http.MethodDelete, fmt.Sprintf("http://example.com/v3/accounts/%s", tt.id), "")
			test.AssertHTTPStatus(t, &recorder, tt.status)
		})
	}
}
...
func (suite *TestSuiteStandard) TestAccountsGetSorted() {
	...
	r := test.Request(suite.T(), http.MethodGet, "http://example.com/v3/accounts", "")
	test.AssertHTTPStatus(suite.T(), &r, http.StatusOK)
...
func (suite *TestSuiteStandard) TestAccountsPagination() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(suite.T(), http.MethodGet, fmt.Sprintf("http://example.com/v3/accounts?offset=%d&limit=%d", tt.offset, tt.limit), "")
			test.AssertHTTPStatus(suite.T(), &r, http.StatusOK)
