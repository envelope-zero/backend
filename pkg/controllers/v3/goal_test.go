...
func (suite *TestSuiteStandard) createTestGoal(t *testing.T, c v3.GoalEditable, expectedStatus ...int) v3.GoalResponse {
	...
	recorder := test.Request(t, http.MethodPost, "http://example.com/v3/goals", requestBody)
	test.AssertHTTPStatus(t, &recorder, expectedStatus...)

	var response v3.GoalCreateResponse
...
func (suite *TestSuiteStandard) TestGoalsOptions() {
	tests := []struct {
		...
		},
		{
			...
			"",
			func() string {
				return suite.createTestGoal(suite.T(), v3.GoalEditable{Amount: decimal.NewFromFloat(31)}).Data.Links.Self
			},
		},
	}
...
func (suite *TestSuiteStandard) TestGoalsDatabaseError() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			recorder := test.Request(t, tt.method, fmt.Sprintf("http://example.com/v3/goals%s", tt.path), tt.body)
...
func (suite *TestSuiteStandard) TestGoalsGet() {
	g1 := suite.createTestGoal(suite.T(), v3.GoalEditable{
	...
	g3 := suite.createTestGoal(suite.T(), v3.GoalEditable{
		Amount: decimal.NewFromFloat(50),
		Month:  types.NewMonth(2024, 2),
		Name:   "Before g2",
	})

	recorder := test.Request(suite.T(), http.MethodGet, "http://example.com/v3/goals", "")

	var response v3.GoalListResponse
...
func (suite *TestSuiteStandard) TestGoalsGetFilter() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			var re v3.GoalListResponse
			r := test.Request(t, http.MethodGet, fmt.Sprintf("/v3/goals?%s", tt.query), "")
...
func (suite *TestSuiteStandard) TestGoalsGetInvalidQuery() {
	...
	for _, tt := range tests {
		suite.T().Run(tt, func(t *testing.T) {
			recorder := test.Request(t, http.MethodGet, fmt.Sprintf("http://example.com/v3/goals?%s", tt), "")
...
func (suite *TestSuiteStandard) TestGoalsCreateInvalidBody() {
	r := test.Request(suite.T(), http.MethodPost, "http://example.com/v3/goals", `{ Invalid request": Body }`)
...
func (suite *TestSuiteStandard) TestGoalsCreate() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, http.MethodPost, "http://example.com/v3/goals", tt.goals)
...
func (suite *TestSuiteStandard) TestGoalsGetSingle() {
	tests := []struct {
	...
	}{
		{
			...
			"",
			func() string {
				return suite.createTestGoal(suite.T(), v3.GoalEditable{Amount: decimal.NewFromFloat(42)}).Data.Links.Self
			},
		},
		{
			"Invalid UUID",
...
func (suite *TestSuiteStandard) TestGoalsDelete() {
	tests := []struct {
	...
	}{
		{
			"Standard deletion",
			http.StatusNoContent,
			suite.createTestGoal(suite.T(), v3.GoalEditable{Amount: decimal.NewFromFloat(2100)}).Data.ID.String(),
		},
		{
			"Does not exist",
			http.StatusNotFound,
...
func (suite *TestSuiteStandard) TestGoalsUpdateFail() {
	goal := suite.createTestGoal(suite.T(), v3.GoalEditable{Amount: decimal.NewFromFloat(170), Note: "Test note for goal"})
	...
	tests := []struct {
		...
		},
		{
			...
			goal.Data.ID.String(),
			http.StatusBadRequest,
			v3.GoalEditable{}, // Sets the EnvelopeID to uuid.Nil
		},
		{
			"Negative amount",
			goal.Data.ID.String(),
...
func (suite *TestSuiteStandard) TestUpdateNonExistingGoal() {
	recorder := test.Request(suite.T(), http.MethodPatch, "http://example.com/v3/goals/c08c0a04-2a12-4cb9-8b4a-87cf270cdd8d", `{ "note": "2" }`)
