...
func (suite *TestSuiteStandard) parseCSV(t *testing.T, accountID uuid.UUID, file string) v3.ImportPreviewList {
	...
	recorder := test.Request(t, http.MethodPost, fmt.Sprintf("http://example.com/v3/import/%s", path), body, headers)
	test.AssertHTTPStatus(t, &recorder, http.StatusOK)

	// Decode the response
	var response v3.ImportPreviewList
...
func (suite *TestSuiteStandard) TestImportSuccess() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			recorder := test.Request(t, http.MethodPost, fmt.Sprintf("http://example.com/v3/import/%s", tt.path), body, headers)
...
func (suite *TestSuiteStandard) TestImportYnab4BudgetCalculation() {
	...
	recorder := test.Request(suite.T(), http.MethodPost, "http://example.com/v3/import/ynab4?budgetName=Test Budget", body, headers)
...
func (suite *TestSuiteStandard) TestImportGet() {
	r := test.Request(suite.T(), http.MethodGet, "http://example.com/v3/import", "")
	test.AssertHTTPStatus(suite.T(), &r, http.StatusOK)

	var links v3.ImportResponse
	test.DecodeResponse(suite.T(), &r, &links)

	assert.Equal(suite.T(), v3.ImportResponse{
		Links: v3.ImportLinks{
