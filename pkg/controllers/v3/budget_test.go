...
func (suite *TestSuiteStandard) createTestBudget(t *testing.T, c v3.BudgetEditable, expectedStatus ...int) v3.BudgetResponse {
	...
	r := test.Request(t, http.MethodPost, "http://example.com/v3/budgets", body)
	test.AssertHTTPStatus(t, &r, expectedStatus...)

	var a v3.BudgetCreateResponse
...
func (suite *TestSuiteStandard) TestBudgetsDBClosed() {
	tests := []struct {
		...
		},
		{
			"GET fails",
			func(t *testing.T) {
				recorder := test.Request(t, http.MethodGet, "http://example.com/v3/budgets", "")
...
func (suite *TestSuiteStandard) TestBudgetsGetSingle() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, tt.method, fmt.Sprintf("http://example.com/v3/budgets/%s", tt.id), "")

			var budget v3.BudgetResponse
...
func (suite *TestSuiteStandard) TestBudgetsGetFilter() {
	...
	var re v3.BudgetListResponse
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, http.MethodGet, fmt.Sprintf("http://example.com/v3/budgets?%s", tt.query), "")
...
func (suite *TestSuiteStandard) TestBudgetsCreateFails() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, http.MethodPost, "http://example.com/v3/budgets", tt.body)
			test.AssertHTTPStatus(t, &r, http.StatusBadRequest)

			var b v3.BudgetCreateResponse
...
func (suite *TestSuiteStandard) TestBudgetsUpdate() {
	...
	var updatedBudget v3.BudgetResponse
...
func (suite *TestSuiteStandard) TestBudgetsUpdateFails() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			// Update budget
			recorder = test.Request(t, http.MethodPatch, fmt.Sprintf("http://example.com/v3/budgets/%s", tt.id), tt.body)
...
func (suite *TestSuiteStandard) TestBudgetsDelete() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			// Update budget
			recorder = test.Request(t, http.MethodDelete, fmt.Sprintf("http://example.com/v3/budgets/%s", tt.id), "")
			test.AssertHTTPStatus(t, &recorder, tt.status)
		})
	}
}
...
func (suite *TestSuiteStandard) TestBudgetsGetSorted() {
	...
	r := test.Request(suite.T(), http.MethodGet, "http://example.com/v3/budgets", "")
	test.AssertHTTPStatus(suite.T(), &r, http.StatusOK)
...
func (suite *TestSuiteStandard) TestBudgetsPagination() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(suite.T(), http.MethodGet, fmt.Sprintf("http://example.com/v3/budgets?offset=%d&limit=%d", tt.offset, tt.limit), "")
			test.AssertHTTPStatus(suite.T(), &r, http.StatusOK)
