...
func (suite *TestSuiteStandard) createTestMatchRule(t *testing.T, c models.MatchRuleCreate, expectedStatus ...int) v3.MatchRuleResponse {
	...
	r := test.Request(t, http.MethodPost, "http://example.com/v3/match-rules", rules)
	test.AssertHTTPStatus(t, &r, expectedStatus...)

	var res v3.MatchRuleCreateResponse
...
func (suite *TestSuiteStandard) TestMatchRuleCreate() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, http.MethodPost, "http://example.com/v3/match-rules", tt.create)
			test.AssertHTTPStatus(t, &r, tt.expectedStatus)

			var tr v3.MatchRuleCreateResponse
	...
	tests := []struct {
		...
		},
		{
			...
			func(t *testing.T) string {
				return suite.createTestMatchRule(t, models.MatchRuleCreate{
...
func (suite *TestSuiteStandard) TestMatchRulesDatabaseError() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			recorder := test.Request(t, tt.method, fmt.Sprintf("http://example.com/v3/match-rules%s", tt.path), tt.body)
...
func (suite *TestSuiteStandard) TestMatchRulesGetFilter() {
	...
	_ = suite.createTestMatchRule(suite.T(), models.MatchRuleCreate{
		Priority:  1,
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			var re v3.MatchRuleListResponse
			r := test.Request(t, http.MethodGet, fmt.Sprintf("/v3/match-rules?%s", tt.query), "")
...
func (suite *TestSuiteStandard) TestMatchRulesCreateInvalidBody() {
	r := test.Request(suite.T(), http.MethodPost, "http://example.com/v3/match-rules", `{ Invalid request": Body }`)
...
func (suite *TestSuiteStandard) TestMatchRulesCreate() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			r := test.Request(t, http.MethodPost, "http://example.com/v3/match-rules", tt.matchRules)
			test.AssertHTTPStatus(t, &r, tt.expectedStatus)
...
func (suite *TestSuiteStandard) TestMatchRulesGetSingle() {
	tests := []struct {
	...
	}{
		{
			...
			"",
			func(t *testing.T) string {
				return suite.createTestMatchRule(t, models.MatchRuleCreate{AccountID: suite.createTestAccount(t, models.Account{}).Data.ID}).Data.Links.Self
			},
		},
		{
			"Invalid UUID",
...
func (suite *TestSuiteStandard) TestMatchRulesUpdateFail() {
	m := suite.createTestMatchRule(suite.T(), models.MatchRuleCreate{
...
func (suite *TestSuiteStandard) TestMatchRulesDelete() {
	tests := []struct {
	...
	}{
		{
			"Standard deletion",
			http.StatusNoContent,
			suite.createTestMatchRule(suite.T(), models.MatchRuleCreate{
				AccountID: suite.createTestAccount(suite.T(), models.Account{}).Data.ID,
				Match:     "Some match*",
			}).Data.ID.String(),
		},
...
func (suite *TestSuiteStandard) TestMatchRulesGetSorted() {
	...
	m2 := suite.createTestMatchRule(suite.T(), models.MatchRuleCreate{
		Priority:  2,
		Match:     "*Match the Second Account",
		AccountID: a.Data.ID,
	})

	m3 := suite.createTestMatchRule(suite.T(), models.MatchRuleCreate{
		Priority:  1,
		Match:     "Exact match",
		AccountID: a.Data.ID,
	})

	m4 := suite.createTestMatchRule(suite.T(), models.MatchRuleCreate{
		Priority:  3,
		Match:     "Coffee Shop*",
		AccountID: a.Data.ID,
	})

	m5 := suite.createTestMatchRule(suite.T(), models.MatchRuleCreate{
		Priority:  3,
		Match:     "Coffee Shop",
		AccountID: a.Data.ID,
	})
	...
	var re v3.MatchRuleListResponse
	r := test.Request(suite.T(), http.MethodGet, "/v3/match-rules", "")
