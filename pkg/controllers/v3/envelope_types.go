...
func newEnvelope(c *gin.Context, model models.Envelope) Envelope {
	...
	return Envelope{
		...
		Links: EnvelopeLinks{
			Self:         fmt.Sprintf("%s/v3/envelopes/%s", url, model.ID),
			Transactions: fmt.Sprintf("%s/v3/transactions?envelope=%s", url, model.ID),
			Month:        fmt.Sprintf("%s/v3/envelopes/%s/YYYY-MM", url, model.ID),
		},
	}
}
