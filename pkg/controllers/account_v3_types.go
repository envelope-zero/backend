...
func (editable AccountV3Editable) model() models.Account {
...
type AccountV3 struct {
	models.DefaultModel
	AccountV3Editable
	Links AccountV3Links `json:"links"`
...
func newAccountV3(c *gin.Context, db *gorm.DB, model models.Account) (AccountV3, httperrors.Error) {
	url := c.GetString(string(database.ContextURL))

	account := AccountV3{
		DefaultModel: model.DefaultModel,
		AccountV3Editable: AccountV3Editable{
			Name:               model.Name,
			Note:               model.Note,
		...
		Links: AccountV3Links{
			Self:         fmt.Sprintf("%s/v3/accounts/%s", url, model.ID),
			Transactions: fmt.Sprintf("%s/v3/transactions?account=%s", url, model.ID),
		},
	}
...
func (f AccountQueryFilterV3) model() (models.Account, httperrors.Error) {
