...
func TestParse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			f, err := os.OpenFile(fmt.Sprintf("../../../../test/data/importer/ynab-import/%s", tt.file), os.O_RDONLY, 0o400)
			if err != nil {
...
func TestReadError(t *testing.T) {
	f, err := os.OpenFile(fmt.Sprintf("../../../../test/data/importer/ynab-import/%s", "comdirect-ynap.csv"), os.O_RDONLY, 0o400)
	if err != nil {
		assert.FailNow(t, "Failed to open the test file", err)
	}
